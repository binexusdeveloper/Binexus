...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BinexusTEST2\r");
            Send(hSocket, "WHO #BinexusTEST2\r");
        } else {
            // randomly join #Binexus00-#Binexus05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Binexus%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Binexus%02d\r", channel_number).c_str());
        }
